                ...
                app[_routes[route].method]('/' + route, function(req, res) {
                    ...
                    _this.handle(req, routeInfo, function(err, data) {
                        if(err !== null) {
                            ...
                            } else if(typeof err === 'object' && typeof err.error === 'undefined') {
                                err = {error: { message: err.message }};
                            }
                            sendContent(res, err, 400);
                        } else {
                            sendContent(res, data, 200);
                        }
                    });
                });
            }
        ...
        if(object instanceof Array) {
            for(var a in object) {
                if(object.hasOwnProperty(a)) {
                    contentLength += new Buffer(object[a]).length;
                }
            }
        } else {
            contentLength = new Buffer(object).length;
        }
